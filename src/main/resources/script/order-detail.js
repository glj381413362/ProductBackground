$(document).ready(function () {
    for (let j = 0; j < $(".sc-detail-allshow").length; j++) {
        for (let i = 0; i < 3; i++) {
            $($($(".sc-detail-allshow")[j]).parent().find("div")[i]).removeClass("hidden")
        }//默认显示前三条物流信息
    }
...
$(".sc-detail-allshow").click(function () {
    if ($(this).children("span").first().text() === "查看全部") {
        ...
        for (let i = 3; i < $(this).parent().parent().parent().find(".sc-detail-expressstate div").length; i++) {
            //前三显示，之后的隐藏
            $($(this).parent().parent().parent().find(".sc-detail-expressstate div")[i]).addClass("hidden")

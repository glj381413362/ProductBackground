        ...
        bindEvent: function (totalsubpageTmep, args) {
            return (function () {
                totalsubpageTmep.on("click", "a.geraltTb_pager", function (event) {
                    ...
                    if(obj.length>0){
                        if($(obj[0]).hasClass("glyphicon-chevron-left") && togglecurrPage>1){
                            togglecurrPage=togglecurrPage-1;
                        }else if($(obj[0]).hasClass("glyphicon-chevron-right") && togglecurrPage<args.totalPage){
                            togglecurrPage=togglecurrPage+1;
                        }
                    }else {
...
function currentPageActived(page){
    ...
    for(let page_i=0;page_i<page_arr.length;page_i++){
        ...
        if(page===page_item){
            ...
            if((!isNaN(page_item)) && $(page_arr[page_i]).hasClass("current-page-actived")){

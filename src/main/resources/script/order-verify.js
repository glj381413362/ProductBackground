$(document).ready(function () {
    ...
    $(".sc-order-custominfo").first().addClass("customer-bdactive").append("<span class=\"sc-order-defaultaddress color-fff font12\">默认</span>\n");
...
$(".sc-order-mbshowmore").click(function () {
    ...
    let  isBdactive=$(".sc-order-addresslist").find(".sc-order-custominfo").hasClass("customer-bdactive")

    if(!isBdactive){
        $(".sc-order-addresslist").find(".sc-order-custominfo").first().addClass("customer-bdactive")
    }
    $(".sc-order-addresslist").find(".sc-order-custominfo").first().append("<span class=\"sc-order-defaultaddress color-fff font12\">默认</span>\n");
})
//移动端隐藏更多地址
...
$(".sc-order-addresspc-set").click(function () {
    ...
    $(this).find("p").append("<img src=\"../assets/icon/edited.svg\" class=\"fr pd-R15 edited\">")
...
function isSelfFetch() {
    if ($(".sc-order-selffetch").find("input").prop("checked")) {//勾选
        $(".sc-order-addresspc-set").removeClass("bd-active").find("input").prop("checked", false)
...
$(".sc-order-checked").click(function () {
    if ($(this).prop("checked")) { //true
        $(this).parent().css({"background-image": "url(\'../assets/icon/bg-right.svg\')", "border": "none"})
    } else {
        $(this).parent().css({"background-image": "url('')", "border": "1px solid #000000"})
    }
});
//优惠券下拉框展示时，加上透明遮罩
...
$(".show-invoice").click(function () {
    $(this).find(".invoice-checked").prop("checked") ?
        $(".sc-order-invoice").children(".sc-order-invoice-show").removeClass("hidden") :
        $(".sc-order-invoice").children(".sc-order-invoice-show").addClass("hidden")

})//选中开具发票，显示具体发票信息
// 弹出修改收货地址
$(".sc-order-addresspc-set,.sc-order-custominfo").on("click", ".seteditaddress", function () {
    ...
    let addressdetail = $(this).parent().parent().find(".customer-addressdetail").text()

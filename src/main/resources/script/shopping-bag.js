...
$(".sc-list-delete").click(function () {
    ...
    let totalprice = parseFloat($(this).closest(".row").find(".totalprice").first().text())
    alltotalprice -= totalprice//总价再计算
    $(".alltotalprice").text(alltotalprice)
    if ($(this).parent("div").parent("div").parent("div").find(".sc-list-content1").length==1) {
...
$(".sc-list-checked").click(function () {
    if ($(this).prop("checked")) { //true
        ...
        $(this).parent().parent().parent().parent().find(".sc-list-listchecked").prop({  //隐式迭代
            checked: false
            //设置当前品牌父类勾选checked状态
        }).parent().css({"background-image": "url('')", "border": "1px solid #000000"})
...
$(".sc-list-listchecked").click(function () {
    ...
    if ($(this).prop("checked")) { //true
        ...
        $(this).parent().parent().parent().parent().find(".sc-list-checked").prop({  //隐式迭代
            checked: true
            //设置子类勾选checked状态
        }).parent().css({"background-image": "url(\'../assets/icon/bg-right.svg\')", "border": "none"})
        //设置子类勾选样式
    } else {
        $(this).parent().css({"background-image": "url('')", "border": "1px solid #000000"})
...
$(window).scroll(function () {
    ...
    $(window).scroll(function () {
        //滑到接近底部
        if ($(window).scrollTop() +document.documentElement.clientHeight + 120 >= $(document.body).height()) {
